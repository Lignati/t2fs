...
int createDirEntry(int dirInodeNumber,char * fileLastName,int numeroInode,int typeVal){
	...
	printf("criando Entrada %s em %d\n",fileLastName,dirInodeNumber);
	if(diretorioInode.blocksFileSize == 0){
		
		printf("cria um bloco\n");
		createDataBlock(dirInodeNumber,0);
		printf("1 bytes file size %d\n",diretorioInode.blocksFileSize);
		diretorioInode = leInode(dirInodeNumber);
		emptyDir(diretorioInode.dataPtr[0],dirInodeNumber);
		printf("2 bytes file size %d\n",diretorioInode.blocksFileSize);
		diretorioInode = leInode(dirInodeNumber);
		printf("3 bytes file size %d\n",diretorioInode.blocksFileSize);
	}
	printf("4 bytes file size %d\n",diretorioInode.blocksFileSize);
	if(diretorioInode.blocksFileSize > 0){
		carregaBloco(diretorioInode.dataPtr[0]);
		for(i = 0; i < numeroRecords; i++) {
	...
	if(diretorioInode.blocksFileSize > 2){
		...
		printf("single indir free record %d\n",singleIndirFreeRecord);
		for(i = 0; i < numeroRecords; i++) {
...
int mkdir2 (char *pathname) {
	...
	switch(createFilePathParser(pathname,dirName,fileLastName)){
		...
		printf("dir Inode Number %d this inode number %d\n",dirInodeNumber,numeroInode);
