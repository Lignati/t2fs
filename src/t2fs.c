...
struct t2fs_record  procuraDirEntryDuplaIndirecao(int blocoIndireto,int numeroInterno,int numeroExterno){
	int i,bloco, enderecoBloco;
	struct t2fs_record record;	
	carregaBloco(blocoIndireto);
	memcpy((void*)&enderecoBloco,(void *)&blocoAtual[bloco / (tamanhoBlocoBytes/sizeof(DWORD))],sizeof(DWORD));
	bloco = bloco % (tamanhoBlocoBytes/sizeof(DWORD));
...
struct t2fs_record  removeRecords(int bloco,char* partialPath,int type){
	...
	for(i = 0; i < numeroRecords; i++) {
		...
		if(record.TypeVal == TYPEVAL_REGULAR && strcmp(partialPath,record.name)){
...
void leBytesBloco(int * bytesRestantes, int size, char * buffer,int bloco, int * contador,int byteInicial){
	...
	carregaBloco(bloco);
	j = byteInicial;
	for(; j < tamanhoBlocoBytes && *contador < size && *contador < *bytesRestantes;*contador++,j++){
...
void readArquivoIndirecao(int ptrIndirecao ,int blocoInicial,int * bytesRestantes, int size, char * buffer,int * contador,int handle){
	int k,bytesLidos,byteInicial,numeroBloco;
	struct t2fs_record record;	
...
void readArquivoDuplaIndirecao(int ptrIndirecao ,int blocoInicial,int * bytesRestantes, int size, char * buffer,int * contador,int handle){
	int k,bytesLidos,byteInicial,ptrDuplaIndirecao;
	struct t2fs_record record;
	int ptrInicial;
...
int createDataBlockDoubleIndir(int numeroBlocoInd, int numeroInternoBloco,int inodeNumber){
	...
	if((numeroInternoBloco - tamanhoBloco+2)%tamanhoBloco == 0)	
		memcpy((void*) &(blocoAtual[((numeroInternoBloco-2-tamanhoBloco)/tamanhoBloco)*sizeof(DWORD)]),
			(void*)&numeroNovoBloco,
			sizeof(DWORD));
			escreveBloco(numeroBlocoInd);

	memcpy((void*)&numeroBlocoSingleIndir,
	&(blocoAtual[((numeroInternoBloco-2-tamanhoBloco)/tamanhoBloco)*sizeof(DWORD)]),
	sizeof(DWORD));
	

	return createDataBlockSingleIndir(numeroBlocoSingleIndir,numeroInternoBloco,inodeNumber);
		...
		for(i = 0; i < tamanhoBlocoBytes/sizeof(DWORD);i++){
			...
			if(blocoAtual[i*sizeof(DWORD)] != INVALID_PTR)
				...
				if(blocoNovoRecord > 0)
					return blocoRetorno;
			if(blocoAtual[i*sizeof(DWORD)] == INVALID_PTR){
				blocoNovoRecord = createDataBlockDoubleIndir(inodeNumber,(i*sizeof(DWORD)),inodeNumber);			
				emptyDir(blocoNovoRecord);
				return blocoNovoRecord;
			}	
	}
...
int createDataBlock(int numeroInode,int numeroInternoBloco){
	...
	if((numeroInternoBloco < (tamanhoBlocoBytes/sizeof(DWORD) + 2)) && (numeroInternoBloco >=2)){
		createDataBlockSingleIndir(inode.singleIndPtr,numeroInternoBloco,numeroInode);
		inode.blocksFileSize += 1;	
	}
...
int createDirEntry(int dirInodeNumber,char * fileLastName,int numeroInode,int typeVal){
	...
	printf("agora estou tentando criar essa file:%s no inode %d, meu inode eh %d data ptr[0] eh  %d\n",fileLastName,dirInodeNumber,numeroInode,diretorioInode.dataPtr[0]);
	if(diretorioInode.blocksFileSize == 0){
...
struct t2fs_record  deletaBlocoSingleIndir(int blocoIndireto){
	int i,bloco;
	struct t2fs_record record;	
	for(i = 0; i < tamanhoBlocoBytes/sizeof(DWORD);i++){
		carregaBloco(blocoIndireto);
...
struct t2fs_record  deletaBlocoDoubleIndir(int blocoIndireto){
	int i,bloco;
	struct t2fs_record record;	
	for(i = 0; i < tamanhoBlocoBytes/sizeof(DWORD);i++){
		carregaBloco(blocoIndireto);
